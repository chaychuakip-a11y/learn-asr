...
满足以下条件后进入 [音频零基础 6 节桥梁课](音频零基础课程索引.md)：
...
基础 6 完成后，先用 6 节音频桥梁课建立振动、周期、RMS、dB、PCM、位深、通道、谐波、SNR 和真实 WAV 审计的直觉，再从 ASR 主线第 1 课学习采样定理与混叠；到第 7～9 课时，会把两类基础能力用于真正的声学编码器。
