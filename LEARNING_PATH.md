...
- 学习材料：[`认知工具箱总手册`](COGNITIVE_TOOLKIT.md)与 [`认知拓展课程索引`](notebooks/认知拓展课程索引.md)。
- 重点：论证与反例、自然频数与贝叶斯、因果图、存量/反馈/延迟、信息价值、熵与互信息、预测校准、提取练习与迁移。
- 建议时间：第一遍 12～16 小时，随后用 12 周把工具迁移到真实问题。
- 安排：阶段 0 后完成 01～02；第一次实验比较前完成 03；进入流式/部署前完成 04～05；学习语言模型时完成 06；从现在开始持续做 07 的预测日志和 08 的间隔提取。
- 离场标准：能为一个新问题构造反例、写基础率、画因果图和反馈回路，比较行动与信息价值，并留下可到期评分的概率判断。

这条路线是跨领域选修，不增加 ASR 技术先修条件；它训练的是在陌生领域中提问、更新和迁移的能力。

### 进阶贯穿阶段：逻辑与信息研究 8 课

- 学习材料：[`逻辑与信息研究进阶手册`](LOGIC_INFORMATION_RESEARCH.md)、[`课程索引`](notebooks/逻辑与信息研究进阶课程索引.md)和 [`研究工作台`](research_workspace/README.md)。
- 重点：问题定义、必要/充分条件、量词与作用域、演绎/归纳/溯因、论证图与钢人化、分面布尔检索、横向阅读、来源家族、证据矩阵、冲突消解和透明研究档案。
- 建议时间：第一遍 16～24 小时，再用一个真实问题完成 12～20 小时的研究档案。
- 安排：完成认知拓展 01 后学研究进阶 01～03；第一次技术调研前学 04～06；需要形成调研结论时完成 07～08。
- 离场标准：能保存完整检索式与排除理由，分开报告数和独立来源家族，主动寻找反方证据，并输出带置信度、适用边界、最大遗漏风险和更新触发器的结论。

这条路线不能用“读过很多文章”验收。最终产物是一份别人能够复查、指出断链并在新证据出现后更新的研究档案。
