...
|---|---:|---:|---:|---|
| 第一遍完成 | 205～280 小时 | 26～35 周 | 18～24 周 | 能运行、能复述主要概念 |
| 独立实现 | 355～500 小时 | 45～63 周 | 30～42 周 | 能闭卷实现、排错和评测教学系统 |
| 扎实掌握 | 495～720 小时 | 62～90 周 | 42～60 周 | 能迁移到新数据并完成工程验收 |

推荐目标：**用 40～52 周，每周 10～12 小时完成第一轮掌握**。不要用“连续观看时间”计算进度，只计算亲手推导、编码、实验、复习和答辩的时间。

## 3. 十三个学习阶段（阶段 0～12）

### 贯穿阶段：研究与工程思维 6 课

- 学习材料：[`研究与工程思维课程索引`](notebooks/研究与工程思维课程索引.md)与 [`学习拓展调研`](LEARNING_EXPANSION_RESEARCH.md)。
- 重点：可证伪假设、错误切片、对照/消融、Bootstrap 不确定性、因果排错、Pareto 与风险决策。
- 建议时间：第一遍 12～18 小时，综合任务 8～14 小时。
- 安排：阶段 0 后做第 1 课；阶段 2 后做第 2 课；第一次模型比较前做第 3～4 课；流式/部署排错前做第 5 课；任何上线或选型前做第 6 课。
- 离场标准：任何技术结论都能写成“主张—证据—最强替代解释—反证—边界—下一步”，并能说明总分、相关性或一次成功运行为什么不足以证明原因。

这条课贯穿全部阶段，不是学完即丢的独立知识岛。CTC、前端、泛化、前沿模型和真实项目的实验报告都要复用这六种判断工具。
