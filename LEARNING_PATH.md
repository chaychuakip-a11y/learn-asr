...
|---|---:|---:|---:|---|
| 第一遍完成 | 180～240 小时 | 23～30 周 | 15～20 周 | 能运行、能复述主要概念 |
| 独立实现 | 320～450 小时 | 40～56 周 | 27～38 周 | 能闭卷实现、排错和评测教学系统 |
| 扎实掌握 | 450～650 小时 | 56～81 周 | 38～54 周 | 能迁移到新数据并完成工程验收 |
...
### 阶段 2：声音、采样、FFT、STFT 与 Log-Mel

- 学习材料：主线 01～06。
- 重点：采样率、Nyquist、幅值、dB 参考值、frame/window/hop、DFT/FFT、STFT、Mel、Log-Mel。
- 建议时间：24～36 小时。
- 必做实验：同一条真实音频分别画波形、单帧频谱、线性声谱图和 Log-Mel；改变采样率、窗长、hop 并预测变化。
- 必做实现：`sample_count`、分帧、窗函数、FFT 频率轴、Log-Mel 前端。
- 离场标准：能从 `wav [N]` 手算到特征 `[T,F]` 的每一步 shape、单位和时间分辨率。
...
2. **概念 20～30 分钟**：只学一个因果链，写出输入、输出、shape、单位；
3. **预测 10 分钟**：运行前画图或预测数值；
4. **实验 30～45 分钟**：每次只改一个变量，保留失败结果；
5. **编码 20～40 分钟**：隐藏答案，从空白 cell 写核心函数；
6. **复盘 10 分钟**：在学习日志写“原判断—证据—正确规则”；
7. **间隔复习**：第 1、7、30 天各做一次不看答案的短测。
